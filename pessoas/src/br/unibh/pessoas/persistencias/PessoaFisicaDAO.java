package br.unibh.pessoas.persistencias;
...
{
	private SimpleDateFormat df = new SimpleDateFormat(yyyy-yy-dd)();
		...
		try
		{
			...
			if (res.next())
				...
				(
						...
						p.setString(3, t.Telefone());
						p.setString(4, t.geCpf());
						p.setString(5, t.Email());
						p.setString(6, df.format(t.getDataNascimento()));
						p.setString(7, x);						res.getString("nome"),
	...
	public void insert(PessoaFisica t) 
	{
		try
		{
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement("insert into tb_pessoa_fisica (nome, endereco, telefone, cpf, email, data_nascimento, sexo) values =(?,?,?,?,?,?,?");
